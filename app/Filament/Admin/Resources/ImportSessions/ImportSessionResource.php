...
class ImportSessionResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table->defaultSort('created_at', 'desc')->columns([
            Tables\Columns\TextColumn::make('id')->label('Sesión')->formatStateUsing(fn ($state) => '#'.$state)->sortable(),
            ...
            Tables\Columns\TextColumn::make('total_files')->label('Archivos')->numeric()->sortable(),
            Tables\Columns\TextColumn::make('completed_files')->label('Completados')->numeric()->sortable()->color('success'),
            Tables\Columns\TextColumn::make('duplicate_files')->label('Duplicados')->numeric()->sortable(),
            Tables\Columns\TextColumn::make('failed_files')->label('Fallidos')->numeric()->sortable()->color('danger'),
            Tables\Columns\TextColumn::make('imported_rows')->label('Filas nuevas')->numeric()->sortable(),
            Tables\Columns\TextColumn::make('error_rows')->label('Errores')->numeric()->sortable(),
            Tables\Columns\TextColumn::make('created_at')->label('Inicio')->dateTime('d/m/Y H:i')->sortable(),
        ])->actions([
            ...
            Tables\Actions\Action::make('reprocess')->label('Reprocesar sesión')->icon('heroicon-o-arrow-path')
                ->requiresConfirmation()->modalDescription('Reprocesar vuelve a leer todos los archivos, reemplaza sus filas derivadas y recalcula catálogo, pagos y errores. No borra los archivos originales; cada archivo se reconstruye de forma atómica y los demás continúan si uno falla.')
                ->action(function (ImportSession $record): void {
                    ...
                    Notification::make()->title('Reprocesado finalizado')
                        ->body("{$session->completed_files} archivos correctos y {$session->failed_files} fallidos; {$session->imported_rows} filas reconstruidas.".($session->notes ? " {$session->notes}" : ''))
