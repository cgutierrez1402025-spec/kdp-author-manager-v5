            ...
            <a href="/admin/tasks/{{ $task['id'] }}/edit" class="block rounded-xl border p-3 transition hover:bg-gray-50 dark:hover:bg-white/5 {{ $task['is_overdue'] ? 'border-danger-300 dark:border-danger-500/40' : 'border-gray-200 dark:border-white/10' }}">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0 flex-1">
                        ...
                        @if($task['work_title'])
                            <p class="editorial-title text-sm text-gray-500 dark:text-gray-400">{{ $task['work_title'] }}</p>
                        @endif
                    </div>
                    <span class="shrink-0 whitespace-nowrap rounded-full px-2 py-1 text-xs {{ $task['is_overdue'] ? 'bg-danger-50 text-danger-700 dark:bg-danger-500/10 dark:text-danger-300' : 'bg-gray-100 text-gray-700 dark:bg-white/10 dark:text-gray-300' }}">
                        @if($task['is_overdue'])
                            Vencida
